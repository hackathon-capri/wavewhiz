...
async function handleSubmit() {
  ...
  if (!ok) {
    // erros preenchidos no objeto `errors` e exibidos inline
    return;
  }
  ...
  try {
    ...
    for (const key in form) {
      ...
      if (key === "cpf" && typeof value === "string") value = value.replace(/\D/g, "");
      if (key === "telefone" && typeof value === "string") value = value.replace(/\D/g, "");
      // só anexa campos não vazios (null e "" são ignorados)
      if (value !== null && value !== "") {
        formData.append(key, value);
      }
    }

    // enviar para /usuarios/ (api.baseURL deve cuidar da raiz)
    await api.post('/usuarios/', formData, { headers: { 'Content-Type': 'multipart/form-data' } });

    // exibir feedback mínimo e limpar
    window.alert('Cadastro realizado com sucesso!');
    resetForm();
  } catch (error) {
    console.error('Cadastro error:', error.response?.data || error);
    // se backend retornar erros no formato DRF, exibir mensagens
    const resp = error.response?.data;
    if (resp && typeof resp === 'object') {
      // mapear diretamente para o objeto errors (substitui arrays/strings)
      for (const k in resp) {
        errors[k] = resp[k];
      }
      // se houver non_field_errors ou detail, mostrar alerta resumido
      const summary = resp.non_field_errors || resp.detail || null;
      if (summary) window.alert(String(summary));
    } else {
      window.alert('Erro ao cadastrar, verifique os dados.');
    }
  }
}
</script>
...
input:-webkit-autofill {
  -webkit-box-shadow: 0 0 0 1000px #04384aa6 inset;
